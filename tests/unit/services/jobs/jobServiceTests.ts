...
describe('JobService', () => {
  ...
  describe('processJob', () => {
    ...
    it('should not start the job, resolve false and log a message if the max number of jobs for the provided job type are already running', async () => {
      when(jobStatusService.getInProgressJobs(JobType.GenerateSampleInspection)).thenResolve([
        generateV1Job(),
        generateV1Job(),
        generateV1Job()
      ])

      const result: boolean = await service.processJob(id, JobType.GenerateSampleInspection, maxJobs, getEnvStub)

      assert.isFalse(result)

      verify(jobStatusService.getInProgressJobs(JobType.GenerateSampleInspection)).called()
      verify(logger.log(`Job with type [${JobType.GenerateSampleInspection}] not started as maximum concurrent jobs reached`)).called()
    ...
    it('should generate the job template, start the job and resolve true if the max number of jobs for the provided job type has not yet been reached and the job started successfully', async () => {
      ...
      when(jobStatusService.getInProgressJobs(JobType.GenerateSampleInspection)).thenResolve([])
      when(templateGenerator.generateJobTemplate(id, JobType.GenerateSampleInspection, mockEnvVars, IAM_ROLE)).thenReturn(job)

      const result: boolean = await service.processJob(id, JobType.GenerateSampleInspection, maxJobs, getEnvStub, IAM_ROLE)

      assert.isTrue(result)

      assert.isTrue(getEnvStub.called)
      verify(templateGenerator.generateJobTemplate(id, JobType.GenerateSampleInspection, mockEnvVars, IAM_ROLE)).called()
    ...
    it('should resolve false and log the error if a conflict error occurs when trying to start the job', async () => {
      ...
      verify(k8s.createNamespacedJob(NAMESPACE, job)).called()
      verify(logger.error(`Conflict - Job ${JobType.GenerateSampleInspection}-${id} failed to start as this job is already in progress`)).called()
    })
    ...
    it('should resolve false and log the error if an unhandled exception occurs when trying to start the job', async () => {
      ...
      verify(k8s.createNamespacedJob(NAMESPACE, job)).called()
      verify(logger.error(`Job with type [${JobType.GenerateSampleInspection}] not started due to error`, createJobError)).called()
    })
  })
})
