...
describe('Job1MessageProcessor', () => {
  ...
  describe('process', () => {
    ...
    it('should process the job with the job-specific parameters, including ID field, job type and max number of jobs', async () => {
      ...
      verify(jobService.processJob(JOB_1_ID, JobType.Job1, JOB_1_MAX_JOBS, anyFunction())).called()
    })
    ...
    it('should generate the config map values for the job', async () => {
      ...
      when(jobService.processJob(JOB_1_ID, JobType.Job1, JOB_1_MAX_JOBS, anyFunction())).thenCall((id, type, maxJobs, getEnvFn) => {
    ...
    it('should resolve true when the message is processed successfully', async () => {
      when(jobService.processJob(JOB_1_ID, JobType.Job1, JOB_1_MAX_JOBS, anyFunction())).thenResolve(true)
    ...
    it('should resolve false when the message is processed successfully', async () => {
      when(jobService.processJob(JOB_1_ID, JobType.Job1, JOB_1_MAX_JOBS, anyFunction())).thenResolve(false)
